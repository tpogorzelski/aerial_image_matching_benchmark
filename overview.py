...
list_providers = ['arcgis']

results = []
for matcher in matchers:
    for provider in list_providers:
        for i in range(100):
            results.append(dict(matcher=matcher,
                                provider=provider,
                                inliers_count=random.randint(0,10000),
                                processing_time=0.123+random.random()*10))
        # data = pd.read_csv(dataset_path + matcher + ".csv", sep=";")
        # for inliers_count, processing_time in zip(data['ransac_'+provider], data['time_'+provider]):
        #     results.append(dict(matcher=matcher,
        #                         provider=provider,
